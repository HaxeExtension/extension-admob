...
static const int BANNER_SIZE_ADAPTIVE =
    0; // Anchored adaptive, somewhat default now (a replacement for
       // SMART_BANNER); banner width is fullscreen, height calculated
       // acordingly (might not work well with landscape orientation)
static const int BANNER_SIZE_BANNER = 1; // 320x50
// static const int BANNER_SIZE_FLUID = 2; //A dynamically sized banner that
// matches its parent's width and expands/contracts its height to match the ad's
// content after loading completes. Android donly?
...
- (id)showWithID:(NSString *)ID
        withSize:(NSInteger)size
       withAlign:(NSInteger)align;
- (void)hide;
- (void)align;
...
- (id)showWithID:(NSString *)ID
  ...
  self._root = [[[[UIApplication sharedApplication] delegate] window]
      rootViewController];
  // self._root = [[[UIApplication sharedApplication] keyWindow]
  // rootViewController];
  ...
  case BANNER_SIZE_ADAPTIVE: {
    ...
    if (@available(iOS 11.0, *)) {
      frame = UIEdgeInsetsInsetRect(self._root.view.frame,
                                    self._root.view.safeAreaInsets);
    }
    CGFloat viewWidth = frame.size.width;

    // self._banner = [[GADBannerView alloc]
    // initWithAdSize:GADCurrentOrientationAnchoredAdaptiveBannerAdSizeWithWidth(viewWidth)];
    break;
  }
  ...
  case BANNER_SIZE_MEDIUM_RECTANGLE: {
    self._banner =
        [[GADBannerView alloc] initWithAdSize:GADAdSizeMediumRectangle];
    break;
  }
  }
...
- (void)bannerView:(GADBannerView *)banner
...
- (void)bannerViewDidRecordImpression:(GADBannerView *)banner {
  ...
  [self align]; // repeat align here because might not alaways work at
...
- (id)loadWithAdUnitID:(NSString *)ID {
  ...
  [GADInterstitialAd loadWithAdUnitID:ID
                    ...
                    completionHandler:^(GADInterstitialAd *ad, NSError *error) {
                      if (error != nil) {
                        // NSLog(@"AdMob failed to load interstitial ad with
                        // error: %@", [error localizedDescription]);
                        onStatus(INTERSTITIAL_FAILED_TO_LOAD,
...
- (void)show {
  if (self._ad != nil &&
      [self._ad
          canPresentFromRootViewController:[[[[UIApplication sharedApplication]
                                               delegate] window]
                                               rootViewController]
                                     error:nil]) {
    [self._ad presentFromRootViewController:[[[[UIApplication sharedApplication]
                                                delegate] window]
                                                rootViewController]];
    //[self._ad presentFromRootViewController:[[[UIApplication
...
- (void)ad:(nonnull id<GADFullScreenPresentingAd>)ad
  ...
  onStatus(INTERSTITIAL_FAILED_TO_SHOW,
           [[error localizedDescription] UTF8String]);
}
  ...
  [GADRewardedAd loadWithAdUnitID:ID
                ...
                completionHandler:^(GADRewardedAd *ad, NSError *error) {
                  if (error != nil) {
                    ...
                    onStatus(REWARDED_FAILED_TO_LOAD,
                             [[error localizedDescription] UTF8String]);
                    return;
                  } else {
                    self._ad = ad;
  ...
  if (self._ad != nil &&
      [self._ad
          canPresentFromRootViewController:[[[[UIApplication sharedApplication]
                                     ...
                                     error:nil]) {
    [self._ad
        presentFromRootViewController:[[[[UIApplication sharedApplication]
                                          delegate] window] rootViewController]
             userDidEarnRewardHandler:^{
               GADAdReward *reward = self._ad.adReward;
               NSString *ebat =
                   [NSString stringWithFormat:@"%@:%d", reward.type,
...
// everything is fucked up: in EEA and UK, need to show GDRP message and then,
// if approved, IDFA message
void init(bool testingAds, bool childDirected, bool enableRDP) {
  // NSLog(@"Info init");
  UIViewController *_root = [[[[UIApplication sharedApplication] delegate]
      window] rootViewController];

  //> copy/pasted from here:
  //https://developers.google.com/admob/ios/privacy#objective-c
  ...
  @"948F6324-7875-4599-93DC-6B4E900F25A7" ]; debugSettings.geography =
  UMPDebugGeographyEEA; parameters.debugSettings = debugSettings;*/
  //<<
  ...
  [UMPConsentInformation.sharedInstance
      requestConsentInfoUpdateWithParameters:parameters
                           completionHandler:^(
                             ...
                             // whether it is failed or not, we initialize admob
                             // anyway, because cmon
                             if (requestConsentError) {
                               ...
                               // initMobileAds(testingAds, childDirected,
                               // enableRDP, true);
                             }
                                     ...
                                     _root
                                                          completionHandler:^(
                                                            ...
                                                            if (loadAndPresentError) {
                                                              ...
                                                              // NSLog(@"Error:
                                                              // %@",
                                                              // loadAndPresentError.localizedDescription);
                                                              onStatus(
                                                                  CONSENT_FAILED,
                                                                  [[loadAndPresentError
                                                                      localizedDescription]
  ...
  if (UMPConsentInformation.sharedInstance.canRequestAds) {
    if (hasConsentForPurpose(0) == 1) // consent given, not a best way to check
                                      // it, but don't know any other ways
...
void initMobileAds(bool testingAds, bool childDirected, bool enableRDP,
  ...
  // NSLog(@"Init1 %d %d %d %d", testingAds, childDirected, enableRDP,
  // requestIDFA);
    ...
    if (@available(iOS 14, *)) {
      ...
      [ATTrackingManager requestTrackingAuthorizationWithCompletionHandler:^(
        ...
        [[GADMobileAds sharedInstance]
            startWithCompletionHandler:^(GADInitializationStatus *status) {
...
int hasConsentForPurpose(int purpose) {
  ...
  if (purposeConsents.length > purpose) {
    int hasorwhat = [[purposeConsents
        substringWithRange:NSMakeRange(purpose, 1)] integerValue];
    return hasorwhat;
  }
...
void showPrivacyOptionsForm() {
  ...
  [UMPConsentForm
      presentPrivacyOptionsFormFromViewController:_root
                                completionHandler:^(
                                    NSError *_Nullable formError) {
                                  if (formError) {
                                    onStatus(CONSENT_FAIL,
...
void showBanner(const char *ID, int size, int align) {
  ...
  _bannerListener = [[BannerListener alloc] showWithID:SID
                                              withSize:size
                                             withAlign:align];
}
