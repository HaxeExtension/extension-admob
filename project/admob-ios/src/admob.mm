...
static const int BANNER_SIZE_ADAPTIVE = 0; // Anchored adaptive, somewhat default now (a replacement for SMART_BANNER); banner width is
					   // fullscreen, height calculated acordingly (might not work well with landscape orientation)
static const int BANNER_SIZE_BANNER = 1; // 320x50
// static const int BANNER_SIZE_FLUID = 2; //A dynamically sized banner that matches its parent's width and expands/contracts its height to
// match the ad's content after loading completes. Android donly?
...
{
	if (self._ad != nil &&
	    [self._ad canPresentFromRootViewController:[[[[UIApplication sharedApplication] delegate] window] rootViewController]
						 error:nil])
	{
	...
	[UMPConsentInformation.sharedInstance
	    requestConsentInfoUpdateWithParameters:parameters
				 completionHandler:^(NSError *_Nullable requestConsentError) {
				   ...
				   [UMPConsentForm
				       loadAndPresentIfRequiredFromViewController:root
								completionHandler:^(NSError *loadAndPresentError) {
								  if (loadAndPresentError)
								  {
									  onStatus(CONSENT_FAILED,
										   [[loadAndPresentError localizedDescription] UTF8String]);
								  }

								  if (UMPConsentInformation.sharedInstance.canRequestAds)
								  {
									  if (hasConsentForPuprpose(0) ==
									      1) // consent given, not a best way to check it, but don't
										 // know any other ways
										  initMobileAds(testingAds, childDirected, enableRDP, true);
									  else
										  initMobileAds(
										      testingAds, childDirected, enableRDP, false);
								  }
								}];
				 }];
